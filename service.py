...
TAGS = ['cost', 'detaildesc', 'eventid', 'eventname', 'eventtype', 'evenddt',
        'registrationlink', 'registrationtitle', 'evstartdt']
...
def handler(event, context):
    ...
    try:
        s3.put_object(Bucket=BUCKET, Body=json.dumps(entries), Key='ita.json',
...
def convert_date(row, date_col):
    ...
    """
    try:
        return dt.datetime.strptime(str(row[get_tepp_headers()[date_col]]),
...
def get_first_name(row):
    try:
        return row[get_tepp_headers()['Contact Name (First and Last)']].partition(' ')[0]
    except AttributeError:
        return None
...
def get_last_name(row):
    try:
        return row[get_tepp_headers()['Contact Name (First and Last)']].partition(' ')[-1]
    except AttributeError:
        return None
...
def generate_event_id(row):
    m = hashlib.sha1()
    unique_list = [row[get_tepp_headers()['Event Name']],
...
def get_tepp_events():
    events = []
    worksheet = get_tepp_worksheet()
    permitted_status = ["MOA Received", "Waiting on End of Show Report", "Event Completed"]
